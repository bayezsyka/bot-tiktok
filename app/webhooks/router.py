...
async def _send_initial_reply_background(sender_number: str, inbound_id: str) -> None:
    ...
    try:
        ...
        await client.send_text(
            ...
            idempotency_key=f"tiktok-{inbound_id}-ack",
        )
...
async def handle_farros_wa_webhook(
    ...
    # Resolve canonical URL (following safe redirects up to 5 times)
    canonical_url = await resolve_canonical_tiktok_url(tiktok_url)
    if not canonical_url:
        return WebhookEventResponse(status="ok", message="TikTok URL resolution failed or unsafe")
